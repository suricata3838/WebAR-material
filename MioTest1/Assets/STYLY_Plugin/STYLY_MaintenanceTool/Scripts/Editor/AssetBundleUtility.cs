...
namespace STYLY.MaintenanceTool.Utility
{
    ...
    public class AssetBundleUtility
    {
        ...
        public int RenameAssetForBuild(string path, string guid)
        {
            var nowPath = "Assets/" + path;
            var newPath = nowPath.Substring(0, nowPath.LastIndexOf("/") + 1) + guid + ".prefab";

            Debug.Log("nowPath:" + nowPath + " newPath:" + newPath);
            var result = AssetDatabase.MoveAsset(nowPath, newPath);
            Debug.Log(result);
            return 0;
        }
