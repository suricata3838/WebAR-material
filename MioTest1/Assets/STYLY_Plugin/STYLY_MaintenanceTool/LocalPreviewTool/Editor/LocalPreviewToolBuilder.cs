...
namespace STYLY.MaintenanceTool.Utility
{
    ...
    public class LocalPreviewToolBuilder
    {
        ...
        public LocalPreviewToolBuilder(IEnumerable platformUtils, string targetScene)
        {
            // Setup
        ...
        public void TestBuildAndRunScene()
        {
            ...
            // 互換性維持のため、従来方法でもファイルを出力する
            GenerateJsonAndCopyBuildedAssetData(sceneXml, sceneGuid);
        ...
        /// </summary>
        private void BuildSceneToAssetBundle()
        {
            ...
            foreach (var buildTarget in needBuildTargets)
            {
                ...
                if (buildResult == false)
                {
                    EditorUtility.DisplayDialog("Error.",
                        "Buid asset bundle failed for platform " + abUtility.GetPlatformName(buildTarget) +
                        "\r\nPlease fix compile error.", "OK");
                    return;
                }
            }
        }
        ...
        /// <summary>
        /// シーン情報JSONファイルを生成し、persistentDataPath\STYLY_SCENEへ出力する
        /// AssetBundleをpersistentDataPath\STYLY_ASSETへコピーする。
        /// <para>
        /// このメソッドは将来削除予定のため、利用は非推奨です。
        /// STYLY-VR・STYLY-Core側がテストモード独自プロトコルに対応したら、
        /// このメソッドは削除します。
        /// それまでは、互換性維持のため、残します。
        /// </para>
        /// </summary>
        ...
        [Obsolete]
        private void GenerateJsonAndCopyBuildedAssetData(string sceneXml, string sceneGuid)
        {
            // packageBasePathの生成。PackageManager配下なら上、STYLY_Plugin同梱なら下
            string packageBasePath = PackageManagerUtility.Instance.targetPackagePath
                ?? Path.Combine(Application.dataPath, "STYLY_Plugin", "STYLY_MaintenanceTool");

            // templatePathの生成
            string templatePath = Path.Combine(packageBasePath, "Template", "SceneXMLTemplate.json");

            // シーン情報jsonを生成する。
            var sceneInfoJson = stylyPluginUtility.GenerateSceneInfoJson(sceneXml, templatePath);

            string guid = abUtility.GetGuidFromBuildedAssetData(targetScene);
            // AssetBundleをSTYLYへコピーする。
            foreach (IPlatformUtility util in platformUtils)
            {
                util.CopyBuildedAssetBundleToSTYLY(guid);
                // シーン情報XMLをSTYLYへ保存する。
                util.SaveSceneJsonToSTYLY(sceneInfoJson, sceneGuid);
            }
        }
